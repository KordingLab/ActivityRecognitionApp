...
public class Clip {
    ...
    public void appendValues(double[] values, long timestamp)
            throws ClipException {
        if (values.length != this._dimensions) {
            throw new ClipException("Dimensions do not match. Received "
                    + values.length + ", expected " + this._dimensions + ".");
        } else {
            synchronized (this) {
                if (this._timestamps.size() == 0) {
                    ...
                    while ((timestamp - first > this._windowSize)
                            && (this._timestamps.size() > 1)) {
                    ...
                    if (timestamp - first > this._windowSize) {
                        ...
                        Log.e("PR", "Clip: There has been a gap longer than "
                                + this._windowSize + "ms.");
                    }
